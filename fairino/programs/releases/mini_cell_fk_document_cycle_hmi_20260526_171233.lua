...
-- Variant: fk_document_cycle_hmi
-- Generated: 20260526_171233
...
function hmi_publish_status()
    ...
    ModbusSlaveWriteDO(CELL_BATCH_COMPLETE, 1, {0})
    ...
    ModbusSlaveWriteAO(CELL_ROBOT_HEARTBEAT, 1, {cycle_counter})
end
-- END hmi_modbus.lua

-- BEGIN motion.lua
-- Robot movement routines for the filter/keerklep demo cell.
-- FK_* points are simulator/demo points now and become real taught points later.
...
function pick_filter_motion()
    PTP(FK_FILTER_PICK_APPROACH, 20, -1, 0)
    Lin(FK_FILTER_PICK, 20, -1, 0, 0)
    SetDO(DO_GRIPPER_CLOSE, 1, 0, 0)
    WaitMs(GRIPPER_CLOSE_TIME_MS)
    Lin(FK_FILTER_LIFT, 20, -1, 0, 0)
end

function place_filter_in_clamp_motion()
    PTP(FK_CLAMP_APPROACH, 20, -1, 0)
    Lin(FK_FILTER_PLACE, 20, -1, 0, 0)
    SetDO(DO_GRIPPER_CLOSE, 0, 0, 0)
    WaitMs(GRIPPER_CLOSE_TIME_MS)
    Lin(FK_CLAMP_RETRACT, 20, -1, 0, 0)
end
...
function pick_check_valve_motion()
    PTP(FK_VALVE_PICK_APPROACH, 20, -1, 0)
    Lin(FK_VALVE_PICK, 20, -1, 0, 0)
    SetDO(DO_GRIPPER_CLOSE, 1, 0, 0)
    WaitMs(GRIPPER_CLOSE_TIME_MS)
    Lin(FK_VALVE_LIFT, 20, -1, 0, 0)
end
...
function insert_check_valve_motion()
    PTP(FK_VALVE_PLACE_APPROACH, 20, -1, 0)
    Lin(FK_VALVE_PLACE, 20, -1, 0, 0)
    SetDO(DO_GRIPPER_CLOSE, 0, 0, 0)
    WaitMs(CLAMP_SETTLE_TIME_MS)
    Lin(FK_VALVE_RETRACT, 20, -1, 0, 0)
end
...
function pick_finished_filter_motion()
    PTP(FK_FINISHED_PICK_APPROACH, 20, -1, 0)
    Lin(FK_FINISHED_PICK, 20, -1, 0, 0)
    SetDO(DO_GRIPPER_CLOSE, 1, 0, 0)
    WaitMs(GRIPPER_CLOSE_TIME_MS)
    Lin(FK_FINISHED_LIFT, 20, -1, 0, 0)
end

function place_in_drying_row_motion()
    PTP(FK_DRYING_ROW_APPROACH, 20, -1, 0)
    Lin(FK_DRYING_ROW_PLACE, 20, -1, 0, 0)
    SetDO(DO_GRIPPER_CLOSE, 0, 0, 0)
    WaitMs(GRIPPER_CLOSE_TIME_MS)
    Lin(FK_DRYING_ROW_RETRACT, 20, -1, 0, 0)
end

function index_drying_row_motion()
    PTP(FK_DRYING_ROW_INDEX_START, 20, -1, 0)
    Lin(FK_DRYING_ROW_INDEX_END, 20, -1, 0, 0)
...
-- Glue station logic.
-- Demo version uses FK_GLUE_* points. Real cell keeps these names and updates
-- the taught coordinates.

function glue_move_approach()
    PTP(FK_GLUE_APPROACH, GLUE_ROTATION_SPEED, -1, 0)
end

function glue_move_contact()
    Lin(FK_GLUE_START, GLUE_ROTATION_SPEED, -1, 0, 0)
end

function glue_move_clear()
    Lin(FK_GLUE_RETRACT, GLUE_ROTATION_SPEED, -1, 0, 0)
end
...
function glue_rotate_wrist_placeholder()
    ...
    Lin(FK_GLUE_START, GLUE_ROTATION_SPEED, -1, 0, 0, 1, 0, 0, 0, 0, 0, 300)
end
