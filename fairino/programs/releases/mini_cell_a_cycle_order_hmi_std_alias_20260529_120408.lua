...
-- Variant: a_cycle_order_hmi_std_alias
-- Generated: 20260529_120408
...
function hmi_read_batch_target()
    ...
    local value = ModbusSlaveReadAI(AI50, 1)
    if value == nil then
        return 1
    end
    return value
...
function hmi_start_requested()
    local current = 0
    if HMI_MODBUS_ENABLED == 1 then
        current = hmi_bool(ModbusSlaveReadDI(DI100, 1))
    end
...
function hmi_stop_requested()
    local current = 0
    if HMI_MODBUS_ENABLED == 1 then
        current = hmi_bool(ModbusSlaveReadDI(DI102, 1))
    end
...
function hmi_reset_requested()
    local current = 0
    if HMI_MODBUS_ENABLED == 1 then
        current = hmi_bool(ModbusSlaveReadDI(DI101, 1))
    end
...
function hmi_publish_status()
    ...
    local ready = 0
    local running = 0
    local fault_active = 0
    local safety_ok = 0
    local glue_active = 0
    local clamp_closed = 0
    local gripper_ok = 0
    local batch_target = hmi_read_batch_target()

    if current_state == S30_WAIT_START and fault_code == FAULT_NONE then
        ready = 1
    end
    if program_done == 0 and current_state ~= S30_WAIT_START and fault_code == FAULT_NONE then
        running = 1
    end
    if fault_code ~= FAULT_NONE then
        fault_active = 1
    end
    if sim_input("safety_ok") then
        safety_ok = 1
    end
    if current_state == S110_APPLY_GLUE then
        glue_active = 1
    end
    if current_state >= S80_CLAMP_FILTER and current_state < S140_UNCLAMP_FILTER and fault_code == FAULT_NONE then
        clamp_closed = 1
    end
    if current_state >= S60_PICK_FILTER and current_state < S160_PLACE_IN_DRYING_ROW and fault_code == FAULT_NONE then
        gripper_ok = 1
    end

    ModbusSlaveWriteDO(DO100, 1, {ready})
    ModbusSlaveWriteDO(DO101, 1, {running})
    ModbusSlaveWriteDO(DO102, 1, {fault_active})
    ModbusSlaveWriteDO(DO103, 1, {safety_ok})
    ModbusSlaveWriteDO(DO104, 1, {glue_active})
    ModbusSlaveWriteDO(DO105, 1, {clamp_closed})
    ModbusSlaveWriteDO(DO106, 1, {gripper_ok})
    ModbusSlaveWriteDO(DO107, 1, {0})
    ModbusSlaveWriteDO(DO108, 1, {1})

    hmi_cs = current_state
    hmi_fc = fault_code
    hmi_cc = cycle_counter
    hmi_bt = batch_target

    ModbusSlaveWriteAO(AO50, 1, hmi_cs)
    ModbusSlaveWriteAO(AO51, 1, hmi_fc)
    ModbusSlaveWriteAO(AO52, 1, hmi_cc)
    ModbusSlaveWriteAO(AO53, 1, hmi_bt)
    ModbusSlaveWriteAO(AO54, 1, hmi_cc)
    ModbusSlaveWriteAO(AO55, 1, hmi_cc)
end
-- END hmi_modbus.lua
