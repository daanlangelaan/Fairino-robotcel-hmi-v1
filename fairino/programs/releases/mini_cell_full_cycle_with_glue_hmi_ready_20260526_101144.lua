...
-- Variant: full_cycle_with_glue_hmi_ready
-- Generated: 20260526_101144
...
function hmi_read_coil(alias)
    if HMI_MODBUS_ENABLED ~= 1 then
        return 0
    end

    -- Fairino generated Lua uses ModbusSlaveReadDO(alias, count) for Modbus
    -- TCP slave coil/general digital input reads. Alias names must match the
    -- WebApp Modbus TCP slave register aliases.
    local value = ModbusSlaveReadDO(alias, 1)
    if value == true then
        return 1
    end
    if value == 1 then
...
function hmi_write_discrete(alias, value)
    if HMI_MODBUS_ENABLED ~= 1 then
        return
    end
    ModbusSlaveWriteDO(alias, 1, {value})
end

function hmi_write_input_register(alias, value)
    if HMI_MODBUS_ENABLED ~= 1 then
        return
    end
    ModbusSlaveWriteAO(alias, 1, {value})
end
...
function hmi_start_requested()
    local current = hmi_read_coil("HMI_START_REQ")
...
function hmi_stop_requested()
    local current = hmi_read_coil("HMI_STOP_REQ")
...
function hmi_reset_requested()
    local current = hmi_read_coil("HMI_RESET_REQ")
...
function hmi_publish_status()
    hmi_write_discrete("CELL_READY", current_state == S30_WAIT_START and fault_code == FAULT_NONE and 1 or 0)
    hmi_write_discrete("CELL_RUNNING", program_done == 0 and current_state ~= S30_WAIT_START and fault_code == FAULT_NONE and 1 or 0)
    hmi_write_discrete("CELL_FAULT_ACTIVE", fault_code ~= FAULT_NONE and 1 or 0)
    hmi_write_discrete("CELL_SAFETY_OK", sim_input("safety_ok") and 1 or 0)
    hmi_write_discrete("CELL_GLUE_ACTIVE", current_state == S100_APPLY_GLUE and 1 or 0)

    hmi_write_input_register("CELL_STATE", current_state)
    hmi_write_input_register("CELL_FAULT_CODE", fault_code)
    hmi_write_input_register("CELL_CYCLE_COUNT", cycle_counter)
end
-- END hmi_modbus.lua
