...
-- Variant: a_cycle_order_hmi_cycle_stop_estop
-- Generated: 20260602_112552
...
function guarded_ptp(point, speed)
    if motion_guard() == false then
        return false
    end
    PTP(point, speed, -1, 0)
...
function guarded_lin(point, speed)
    if motion_guard() == false then
        return false
    end
    Lin(point, speed, -1, 0, 0)
    return motion_guard()
end

function move_home()
    return guarded_ptp(A010_HOME, SPEED_HOME)
end

function pick_filter_motion()
    if guarded_ptp(A020_FILTER_PICK_APPROACH, SPEED_APPROACH) == false then return false end
    if guarded_lin(A030_FILTER_PICK, SPEED_PICK_PLACE) == false then return false end
    SetDO(DO_GRIPPER_CLOSE, 1, 0, 0)
    if guarded_wait(GRIPPER_CLOSE_TIME_MS) == false then return false end
    if guarded_lin(A040_FILTER_LIFT, SPEED_RETRACT) == false then return false end
    return true
end

function place_filter_in_clamp_motion()
    if guarded_ptp(A050_CLAMP_APPROACH, SPEED_TRANSPORT) == false then return false end
    if guarded_lin(A060_FILTER_PLACE_IN_CLAMP, SPEED_PICK_PLACE) == false then return false end
    SetDO(DO_GRIPPER_CLOSE, 0, 0, 0)
    if guarded_wait(GRIPPER_CLOSE_TIME_MS) == false then return false end
    if guarded_lin(A070_CLAMP_RETRACT, SPEED_RETRACT) == false then return false end
    return true
end
...
function pick_check_valve_motion()
    if guarded_ptp(A080_VALVE_PICK_APPROACH, SPEED_TRANSPORT) == false then return false end
    if guarded_lin(A090_VALVE_PICK, SPEED_PICK_PLACE) == false then return false end
    SetDO(DO_GRIPPER_CLOSE, 1, 0, 0)
    if guarded_wait(GRIPPER_CLOSE_TIME_MS) == false then return false end
    if guarded_lin(A100_VALVE_LIFT, SPEED_RETRACT) == false then return false end
    return true
end
...
function insert_check_valve_motion()
    if guarded_ptp(A140_VALVE_INSERT_APPROACH, SPEED_APPROACH) == false then return false end
    if guarded_lin(A150_VALVE_INSERT, SPEED_PICK_PLACE) == false then return false end
    SetDO(DO_GRIPPER_CLOSE, 0, 0, 0)
    if guarded_wait(CLAMP_SETTLE_TIME_MS) == false then return false end
    if guarded_lin(A160_VALVE_INSERT_RETRACT, SPEED_RETRACT) == false then return false end
    return true
end
...
function pick_finished_filter_motion()
    if guarded_ptp(A170_FINISHED_PICK_APPROACH, SPEED_TRANSPORT) == false then return false end
    if guarded_lin(A180_FINISHED_PICK, SPEED_PICK_PLACE) == false then return false end
    SetDO(DO_GRIPPER_CLOSE, 1, 0, 0)
    if guarded_wait(GRIPPER_CLOSE_TIME_MS) == false then return false end
    if guarded_lin(A190_FINISHED_LIFT, SPEED_RETRACT) == false then return false end
    return true
end

function place_in_drying_row_motion()
    if guarded_ptp(A200_DRYING_ROW_APPROACH, SPEED_TRANSPORT) == false then return false end
    if guarded_lin(A210_DRYING_ROW_PLACE, SPEED_PICK_PLACE) == false then return false end
    SetDO(DO_GRIPPER_CLOSE, 0, 0, 0)
    if guarded_wait(GRIPPER_CLOSE_TIME_MS) == false then return false end
    if guarded_lin(A220_DRYING_ROW_RETRACT, SPEED_RETRACT) == false then return false end
    return true
end

function index_drying_row_motion()
    if guarded_ptp(A230_DRYING_ROW_INDEX_START, SPEED_DRYING_INDEX) == false then return false end
    if guarded_lin(A240_DRYING_ROW_INDEX_END, SPEED_DRYING_INDEX) == false then return false end
...
function glue_move_approach()
    return guarded_ptp(A110_GLUE_APPROACH, GLUE_ROTATION_SPEED)
end

function glue_move_contact()
    return guarded_lin(A120_GLUE_START, GLUE_ROTATION_SPEED)
end

function glue_move_rotation_end()
    return guarded_lin(A125_GLUE_END, GLUE_ROTATION_SPEED)
end

function glue_move_clear()
    return guarded_lin(A130_GLUE_RETRACT, GLUE_ROTATION_SPEED)
end
